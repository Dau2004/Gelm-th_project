...
def check_password_encryption():
    ...
    if algo != 'pbkdf2_sha256':
        issues.append(f"⚠️  Weak algorithm: {algo} (should be pbkdf2_sha256)")
    else:
    ...
    if iterations < 100000:
        issues.append(f"⚠️  Low iterations: {iterations} (should be 100K+)")
    else:
...
def check_allowed_hosts():
    ...
    if '*' in settings.ALLOWED_HOSTS and not settings.DEBUG:
        ...
        print("⚠️  ALLOWED_HOSTS=['*'] (development only)")
    else:
...
def main():
    ...
    if all_issues:
        print(f"\n⚠️  Found {len(all_issues)} security issues:\n")
        ...
        if settings.DEBUG:
            print("\n⚠️  Note: Running in DEBUG mode (development)")
